...
if(isset($_POST['data-id']) and isset($_POST['state'])){
    ...
    $query_rs_update = "UPDATE portfolio_gallery SET `portfolio_gallery`.`status` = ".$_POST['state']." WHERE `portfolio_gallery`.`id` = ".$_POST['data-id'];
...
$colname_rs_index = "-1";
if (isset($_GET['id'])) {
  $colname_rs_index = $_GET['id'];
}

mysql_select_db($database_conn_database, $conn_database);
$query_rs_index = sprintf("SELECT * FROM portfolio_gallery WHERE portfolio_id = %s ORDER BY rank DESC", GetSQLValueString($colname_rs_index, "int"));
...
mysql_select_db($database_conn_database, $conn_database);
$query_rs_portfolio = sprintf("SELECT * FROM portfolio_localization INNER JOIN languages ON languages.id = portfolio_localization.lang_id WHERE portfolio_id = %d ORDER BY languages.rank DESC LIMIT 1", GetSQLValueString($colname_rs_index , "int"));
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="portlet">
                                <div class="portlet-title">
                                    <div class="caption">
                                       <a href="../portfolio/" style="text-decoration:none;color:#000">Portfolio </a> \ <?php echo $row_rs_portfolio['title1']; ?>'s Gallery
                                    </div>
                                    <div style="float:right;">
                                        <a alt="Back to main page" title="Back to main page" href="../portfolio/index.php" class="btn btn-sm btn-outline dark" ><i class="fa fa-angle-double-left"></i></a>
                                        <a href="insert.php?id=<?php echo $_GET['id']; ?>" alt="Insert" title="Insert" class="btn btn-outline dark btn-sm blue"> <i class="fa fa-plus"></i></a>
                                    </div>
                                </div>
                                    ...
                                    <table class="table table-striped table-bordered table-hover table-header-fixed" id="sample_1" data-rankDarg>
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="no-sort" width="143"><center><strong>Action</strong></center></th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <?php if($totalRows_rs_index > 0){
                                                ...
                                                <tr data-id="<?php echo $row_rs_index['id']; ?>" data-rank="<?php echo $row_rs_index['rank']; ?>">
                                                    ...
                                                    <td style="vertical-align:middle;"><img src="../../images/portfolios/gallery/<?php echo $row_rs_index['image1']; ?>" width="50" />&nbsp;</td>
                                                    ...
                                                    <td style="vertical-align:middle;" align="center" class="nodarg">
                                                        <a alt="Move Up" title="Move Up" href="move_up.php?id=<?php echo $row_rs_index['id']; ?>&rank=<?php echo $row_rs_index['rank']; ?>&portfolio_id=<?php echo $row_rs_index['portfolio_id']; ?>" class="btn btn-outline btn-sm <?php if($counter==1){echo 'disabled default';}else{echo 'yellow';}?>"> <i class="fa fa-angle-double-up"></i></a> 
                                                        <a alt="Move Down" title="Move Down" href="move_down.php?id=<?php echo $row_rs_index['id']; ?>&rank=<?php echo $row_rs_index['rank']; ?>&portfolio_id=<?php echo $row_rs_index['portfolio_id']; ?>" class="btn btn-outline btn-sm <?php if($counter==$totalRows_rs_index){echo 'disabled default';}else{echo 'purple';}?>" > <i class="fa fa-angle-double-down"></i></a>
                                                        <a alt="Edit" title="Edit" href="update.php?id=<?php echo $row_rs_index['id']; ?>&portfolio_id=<?php echo $row_rs_index['portfolio_id']; ?>" class="btn btn-outline btn-sm green"><i class="fa fa-edit"></i></a>
                                                ...
                                                <?php $counter++; }  while ($row_rs_index = mysql_fetch_assoc($rs_index));
                                            }else{ ?>
                                                <tr>
...
<![endif]-->
    ...
    var handleSample = function () {
        ...
        $('.bs_confirmation_delete').on('confirmed.bs.confirmation', function () {
            window.location.href = "delete.php?id=<?php echo $_GET["id"];?>&recordid="+$(this).attr('data-val');
        });
    }
...
mysql_free_result($rs_portfolio);
?>
