...
if(isset($_POST['data-id']) and isset($_POST['state'])){
    ...
    $query_rs_update = "UPDATE testimonial SET `testimonial`.`status` = ".$_POST['state']." WHERE `testimonial`.`id` = ".$_POST['data-id'];
    $rs_update = mysql_query($query_rs_update, $conn_database) or die(mysql_error());
    exit();
}

mysql_select_db($database_conn_database, $conn_database);
$query_rs_index = "SELECT * FROM testimonial ORDER BY rank DESC";
                                            ...
                                            <?php if($totalRows_rs_index > 0){
                                                ...
                                                <tr data-id="<?php echo $row_rs_index['id']; ?>" data-rank="<?php echo $row_rs_index['rank']; ?>">
                                                    ...
                                                    while ($row_rs_lang = mysql_fetch_assoc($rs_lang)){
                                                        mysql_select_db($database_conn_database, $conn_database);
                                                        $query_rs_testimonial = sprintf("SELECT * FROM testimonial_localization tl WHERE tl.lang_id = %d AND tl.testimonial_id = %d", GetSQLValueString($row_rs_lang['id'],'int'),GetSQLValueString($row_rs_index['id'],'int'));
                                                        $rs_testimonial = mysql_query($query_rs_testimonial, $conn_database) or die(mysql_error());
                                                        $row_rs_testimonial = mysql_fetch_assoc($rs_testimonial);
                                                        $totalRows_rs_testimonial = mysql_num_rows($rs_testimonial);
                                                    ?>
                                                    <td style="vertical-align:middle;"><?php echo $row_rs_testimonial['title1']; ?></td>
