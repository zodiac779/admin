...
if(isset($_POST['data-id']) and isset($_POST['state'])){
    ...
    $query_rs_update = "UPDATE service_gallery SET `service_gallery`.`status` = ".$_POST['state']." WHERE `service_gallery`.`id` = ".$_POST['data-id'];
...
mysql_select_db($database_conn_database, $conn_database);
$query_rs_index = sprintf("SELECT * FROM service_gallery WHERE service_id = %s ORDER BY rank DESC", GetSQLValueString($colname_rs_index, "int"));
...
mysql_select_db($database_conn_database, $conn_database);
$query_rs_service = sprintf("SELECT * FROM service_localization INNER JOIN languages ON languages.id = service_localization.lang_id WHERE service_id = %d ORDER BY languages.rank DESC LIMIT 1", GetSQLValueString($colname_rs_index , "int"));
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="portlet">
                                <div class="portlet-title">
                                    <div class="caption">
                                       <a href="../service/" style="text-decoration:none;color:#000">Service </a> \ <?php echo $row_rs_service['title1']; ?>'s Gallery
                                    </div>
                                    <div style="float:right;">
                                        <a alt="Back to main page" title="Back to main page" href="../service/index.php" class="btn btn-sm btn-outline dark" ><i class="fa fa-angle-double-left"></i></a>
                                        ...
                                        <tbody>
                                            <?php if($totalRows_rs_index > 0){
                                                ...
                                                <tr data-id="<?php echo $row_rs_index['id']; ?>" data-rank="<?php echo $row_rs_index['rank']; ?>">
                                                    ...
                                                    <td style="vertical-align:middle;"><img src="../../images/services/gallery/<?php echo $row_rs_index['image1']; ?>" width="50" />&nbsp;</td>
                                                    ...
                                                    <td style="vertical-align:middle;" align="center" class="nodarg">
                                                        <a alt="Move Up" title="Move Up" href="move_up.php?id=<?php echo $row_rs_index['id']; ?>&rank=<?php echo $row_rs_index['rank']; ?>&service_id=<?php echo $row_rs_index['service_id']; ?>" class="btn btn-outline btn-sm <?php if($counter==1){echo 'disabled default';}else{echo 'yellow';}?>"> <i class="fa fa-angle-double-up"></i></a> 
                                                        <a alt="Move Down" title="Move Down" href="move_down.php?id=<?php echo $row_rs_index['id']; ?>&rank=<?php echo $row_rs_index['rank']; ?>&service_id=<?php echo $row_rs_index['service_id']; ?>" class="btn btn-outline btn-sm <?php if($counter==$totalRows_rs_index){echo 'disabled default';}else{echo 'purple';}?>" > <i class="fa fa-angle-double-down"></i></a>
                                                        <a alt="Edit" title="Edit" href="update.php?id=<?php echo $row_rs_index['id']; ?>&service_id=<?php echo $row_rs_index['service_id']; ?>" class="btn btn-outline btn-sm green"><i class="fa fa-edit"></i></a>
