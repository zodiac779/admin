...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "service_gallery")) {

    $allowtype1 = array('bmp', 'gif', 'jpg', 'jpeg', 'gif', 'png');       // allowed file extension
    $max_size = 10000000;                                                 // max file size 10MB
    $savefolder = '../../images/services/gallery/';                       // folder for upload
    $checker1 = true;
    ...
    if($checker1){
        ...
        $updateSQL = sprintf("UPDATE service_gallery SET image1 = %s, alt1 = %s, status = %s, lastupdate = now() WHERE id = %s",
        ...
        while ($row_rs_lang = mysql_fetch_assoc($rs_lang)){
            $updateSQL = sprintf("UPDATE service_gallery_localization SET title1 = %s WHERE service_gallery_id = %s AND lang_id = %s",
...
mysql_select_db($database_conn_database, $conn_database);
$query_rs_update = sprintf("SELECT * FROM service_gallery WHERE id = %s", GetSQLValueString($colname_rs_update, "int"));
...
$colname_rs_service = "-1";
if (isset($_GET['service_id'])) {
  $colname_rs_service = $_GET['service_id'];
}
mysql_select_db($database_conn_database, $conn_database);
$query_rs_service = sprintf("SELECT * FROM service_localization INNER JOIN languages ON languages.id = service_localization.lang_id WHERE service_id  = %d ORDER BY languages.rank DESC LIMIT 1", GetSQLValueString($colname_rs_service, "int"));
$rs_service = mysql_query($query_rs_service, $conn_database) or die(mysql_error());
                ...
                <div class="page-content">
                    ...
                    <div class="portlet light bordered">
                        <div class="portlet-title">
                            <div class="caption">
                                <span class="caption-subject uppercase"><?php echo $row_rs_service['title1']; ?>'s Gallery Update</span>
                            </div>
                            <div class="actions">
                            </div>
                        </div>
                        <div class="portlet-body form">
                            <div class="form-body" style="padding-bottom: 0px;">
                                <form action="<?php echo $editFormAction; ?>" name="service_gallery" id="service_gallery" method="POST" enctype="multipart/form-data">
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="light">
                                                <div class="portlet-body">
                                                    <?php if(isset($Result1)){ ?>
                                                        <div class="alert alert-block alert-success fade in">
                                                            ...
                                                            <h4 class="alert-heading" style="margin-bottom:0px">Your Service gallery has been updated.</h4>
                                                        </div>
                                                    <?php } ?>
                                                    ...
                                                    <?php if(isset($uploadMessage1)){?>
                                                        <div class="alert alert-block alert-danger fade in">
                                                            <button type="button" class="close" data-dismiss="alert"></button>
                                                            <h4 class="alert-heading" style="margin-bottom:0px"><?php echo 'service Gallery Image'.$uploadMessage1; ?></h4>
                                                        </div>
                                                    <?php } ?>                                                  
                                                    ...
                                                    <div class="tab-content">
                                                            ...
                                                            <div class="row">
                                                                <div class="col-md-6">
                                                                    <div class="fileinput fileinput-new" data-provides="fileinput">
                                                                        <label>Service Gallery Image</label>&nbsp;&nbsp;<span style="font-size:11px; color:#0070c0;">Size : 600 x 450 px</span>
                                                                        <div class="input-group ">
                                                                            ...
                                                                            <a href="#" class="input-group-addon btn red fileinput-exists" data-dismiss="fileinput"> Remove </a><?php if($row_rs_update['image1'] != ""){ ?><a href="../../images/services/gallery/<?php echo $row_rs_update['image1']; ?>" class="input-group-addon btn blue fancybox-button" target="_blank"><i class="icon-picture" style="color:#FFF;"></i></a><?php } ?>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                <div class="col-md-6">
                                                                    <div class="form-group">
                                                                        <label>Alternative Text Service Gallery Image</label>
                                                        ...
                                                        do {
                                                        mysql_select_db($database_conn_database, $conn_database);
                                                        $query_rs_service_gallery = sprintf("SELECT * FROM service_gallery_localization pl WHERE pl.lang_id = %d AND pl.service_gallery_id = %d", GetSQLValueString($row_rs_lang['id'],'int'), GetSQLValueString($row_rs_update['id'],'int'));
                                                        $rs_service_gallery = mysql_query($query_rs_service_gallery, $conn_database) or die(mysql_error());
                                                        $row_rs_service_gallery = mysql_fetch_assoc($rs_service_gallery);
                                                        $totalRows_rs_service_gallery = mysql_num_rows($rs_service_gallery);
                                                        if($totalRows_rs_service_gallery==0){
                                                          mysql_select_db($database_conn_database, $conn_database);
                                                          $query_add_service_gallery = sprintf("INSERT INTO service_gallery_localization(lang_id, service_gallery_id) VALUES (%d, %d)",
                                                          GetSQLValueString($row_rs_lang['id'],'int'),GetSQLValueString($row_rs_update['id'],'int'));
                                                          mysql_query($query_add_service_gallery, $conn_database) or die(mysql_error());
                                                        }
                                                        $counter++;
                                                        } while ($totalRows_rs_service_gallery==0 and $counter <= 1) ?>
                                                        ...
                                                        <div class="tab-pane fade <?php if(isset($_POST["current_tab"]) && $_POST["current_tab"] == "tab_1_".$row_rs_lang['lang_abbr']){echo 'active in';}?>" id="tab_1_<?php echo $row_rs_lang['lang_abbr']; ?>">
                                                            ...
                                                            <div class="form-group">
                                                                <label>Service Gallery Title</label>
                                                                <input type="text" name="title1_<?php echo strtolower($row_rs_lang['lang_abbr']); ?>" id="title1_<?php echo strtolower($row_rs_lang['lang_abbr']); ?>" class="form-control" placeholder="" value="<?php echo $row_rs_service_gallery['title1']; ?>"> 
                                                            </div>
                                    ...
                                    <div class="form-actions right">
                                      ...
                                      <a href="index.php?id=<?php echo $row_rs_update['service_id']; ?>" class="btn dark btn-outline" >Back</a>
                                    ...
                                    <input type="hidden" name="MM_update" value="service_gallery">
                                </form>
                                <script>
