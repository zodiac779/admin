...
if ((isset($_GET["id"])) && ($_GET["id"] != "") && (isset($_GET["rank"])) && ($_GET["rank"] != "")) {
  ...
  $query = sprintf("SELECT id, rank FROM languages WHERE rank < %s ORDER BY rank DESC LIMIT 1",
            GetSQLValueString($_GET["rank"], "int"));
    $rs = mysql_query($query, $conn_database) or die(mysql_error());
    $rows = mysql_fetch_assoc($rs);

  $updateSQL = sprintf("UPDATE languages SET rank=%s WHERE id=%s",
