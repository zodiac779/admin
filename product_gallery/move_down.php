...
$insertGoTo = "index.php?id=".$_GET["product_id"]."&status=error";
if ((isset($_GET["id"])) && ($_GET["id"] != "") && (isset($_GET["rank"])) && ($_GET["rank"] != "") && (isset($_GET["product_id"])) && ($_GET["product_id"] != "")) {

  mysql_select_db($database_conn_database, $conn_database);
  $query = sprintf("SELECT id, rank FROM product_gallery WHERE rank < %s AND product_id = %s ORDER BY rank DESC LIMIT 1",
...
$insertGoTo = "index.php?id=".$_GET["product_id"]."&status=success";
}
