...
if(isset($_POST['data-id']) and isset($_POST['state_showhome'])){
    ...
    $query_rs_update = "UPDATE portfolio SET portfolio.".$_POST['data-mode']." = ".$_POST['state_showhome']." WHERE portfolio.id = ".$_POST['data-id'];
    $rs_update = mysql_query($query_rs_update, $conn_database) or die(mysql_error());
    exit();
}

if(isset($_POST['data-id']) and isset($_POST['state_recommend'])){
    mysql_select_db($database_conn_database, $conn_database);
    $query_rs_update = "UPDATE portfolio SET portfolio.".$_POST['data-mode']." = ".$_POST['state_recommend']." WHERE portfolio.id = ".$_POST['data-id'];
    $rs_update = mysql_query($query_rs_update, $conn_database) or die(mysql_error());
    exit();
}
...
if(isset($_POST['data-id']) and isset($_POST['state_status'])){
    mysql_select_db($database_conn_database, $conn_database);
    $query_rs_update = "UPDATE portfolio SET portfolio.".$_POST['data-mode']." = ".$_POST['state_status']." WHERE portfolio.id = ".$_POST['data-id'];
    $rs_update = mysql_query($query_rs_update, $conn_database) or die(mysql_error());
    exit();
}
...
mysql_select_db($database_conn_database, $conn_database);
$query_rs_index = "SELECT * FROM portfolio ORDER BY rank DESC";
                                    ...
                                    <table class="table table-striped table-bordered table-hover table-header-fixed" id="sample_1" data-rankDarg>
                                        <thead>
                                            <tr>
                                                ...
                                                <?php }while ($row_rs_lang = mysql_fetch_assoc($rs_lang)); mysql_data_seek($rs_lang, 0); ?>
                                                <th class="no-sort" width="50"><center><strong>Status</strong></center></th>
                                                <th class="no-sort" width="190"><center><strong>Action</strong></center></th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <?php if($totalRows_rs_index > 0){
                                                ...
                                                <tr data-id="<?php echo $row_rs_index['id']; ?>" data-rank="<?php echo $row_rs_index['rank']; ?>">
                                                    ...
                                                    while ($row_rs_lang = mysql_fetch_assoc($rs_lang)){
                                                        mysql_select_db($database_conn_database, $conn_database);
                                                        $query_rs_portfolio = sprintf("SELECT * FROM portfolio_localization pl WHERE pl.lang_id = %d AND pl.portfolio_id = %d", GetSQLValueString($row_rs_lang['id'],'int'),GetSQLValueString($row_rs_index['id'],'int'));
                                                    ...
                                                    ?>
                                                    <td style="vertical-align:middle;"><?php echo $row_rs_portfolio['title1']; ?></td>
                                                    ...
                                                    <td style="vertical-align:middle;" align="center" class="nodarg">
                                                        ...
                                                        <i data-id="<?php echo $row_rs_index['id']; ?>" data-mode="recommend" class="<?php if($row_rs_index['recommend'] == 1){ echo 'font-yellow-lemon fa fa-star'; }else{ echo 'font-grey-salsa fa fa-star'; } ?> icon-recommend" title="Recommend" style="cursor:default;"></i>
                                                    ...
                                                    <td style="vertical-align:middle;" align="center" class="nodarg">
                                                        ...
                                                        <a alt="Gallery" title="Gallery" href="../portfolio_gallery/index.php?id=<?php echo $row_rs_index['id']; ?>" class="btn btn-outline btn-sm dark"> <i class="fa fa-image"></i></a>
...
jQuery(document).ready(function() {    
   UIConfirmations.init();
            ...
            $(document).find('.icon-recommend[data-id]').on('click', function(){
                console.log('recommend change');
                ...
                $.ajax({
                    type: 'POST',
                    url: 'index.php',
                    data: $('<form>').append($('<input>').prop({'name':'data-id','value':$(this).data('id')})).append($('<input>').prop({'name':'state_recommend','value':$(this).hasClass('font-yellow-lemon fa fa-star') ? 1 : 0})).append($('<input>').prop({'name':'data-mode','value':$(this).attr('data-mode')})).serialize(),
                    success: function(result) {
