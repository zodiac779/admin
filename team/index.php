...
if(isset($_POST['data-id']) and isset($_POST['state'])){
    ...
    $query_rs_update = "UPDATE team SET `team`.`status` = ".$_POST['state']." WHERE `team`.`id` = ".$_POST['data-id'];
    $rs_update = mysql_query($query_rs_update, $conn_database) or die(mysql_error());
    exit();
}

mysql_select_db($database_conn_database, $conn_database);
$query_rs_index = "SELECT * FROM team ORDER BY rank DESC";
                                            ...
                                            <?php if($totalRows_rs_index > 0){
                                                ...
                                                <tr data-id="<?php echo $row_rs_index['id']; ?>" data-rank="<?php echo $row_rs_index['rank']; ?>">
                                                    ...
                                                    while ($row_rs_lang = mysql_fetch_assoc($rs_lang)){
                                                        mysql_select_db($database_conn_database, $conn_database);
                                                        $query_rs_team = sprintf("SELECT * FROM team_localization tl WHERE tl.lang_id = %d AND tl.team_id = %d", GetSQLValueString($row_rs_lang['id'],'int'),GetSQLValueString($row_rs_index['id'],'int'));
                                                    ...
                                                    ?>
                                                    <td style="vertical-align:middle;"><?php echo $row_rs_team['title1']; ?></td>
